...
class Dataset_Custom(Dataset):
    ...
    def get_batch_dates(self, batch_idx):
        if self.calculate_MSE == 0:
            ...
            dates = []
            max_samples = math.ceil((len(self.data_x) - self.seq_len) / self.pred_len)
            for i in range(batch_idx * self.batch_size, min((batch_idx + 1) * self.batch_size, max_samples)):
                pred_s = i * self.pred_len + self.seq_len
                pred_e = pred_s + self.pred_len
                
                if pred_e > len(self.test_dates):
                    pred_e = len(self.test_dates)
                    pred_s = pred_e - self.seq_len
